<template>
  <div class="body" >
    ...
    <h2 v-if="isRecognize">轮廓刻画</h2>
    <img :src="Binary" style="margin-top: 10px" >
    <h2 v-if="isRecognize">轮廓描绘</h2>
    <img :src="Contours" style="margin-top: 10px" >
    <br/>
    <br/>
    <el-upload
      class="upload-demo"
...
export default {
  ...
  data() {
    return {
      imgUrl:  require("@/assets/images/contours.jpg") ,
      isRecognize: false,
      fileList: [],
      resultInfo:'',
      filePath: '',
  ...
  methods: {
    ...
    recognize(){
      ...
      getImg('contours',this.filePath).then(res=>{
        if(res.msg==='error'){
          this.$message.error(res.data);
        }else {
          this.$message.success(res.msg);
          console.log(res)
          let src = res.data.Binary
          src = src == null ? '' : 'data:image/png;base64,' + src
          this.Binary = src;
          src = res.data.Contours;
          src = src == null ? '' : 'data:image/png;base64,' + src
          this.Contours = src;
