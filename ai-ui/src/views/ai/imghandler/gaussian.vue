<template>
  <div class="body" >
    ...
    <h2 v-if="isRecognize">Gaussian_Noise</h2>
    <img :src="Gaussian_Noise" style="margin-top: 10px" >
    <h2 v-if="isRecognize">Gaussian_Blur</h2>
...
export default {
  ...
  data() {
    return {
      imgUrl:  require("@/assets/images/Gaussian.jpg") ,
      isRecognize: false,
      fileList: [],
      resultInfo:'',
      filePath: '',
  ...
  methods: {
    ...
    recognize(){
      ...
      getImg('gaussian',this.filePath).then(res=>{
        if(res.msg==='error'){
          this.$message.error(res.data);
        }else {
          this.$message.success(res.msg);
          console.log(res)
          let src = res.data.Gaussian_Noise
          src = src == null ? '' : 'data:image/png;base64,' + src
          this.Gaussian_Noise = src;
          src = res.data.Gaussian_Blur;
          src = src == null ? '' : 'data:image/png;base64,' + src
