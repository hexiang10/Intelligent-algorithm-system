...
<script>
import { getIdCardImg, getObjImg, upload } from '@/api/ai/utils/AIUtil'

export default {
  Name: "FACE",
  data() {
    return {
      imgUrl:  require("@/assets/images/idcard.jpeg") ,
      isRecognize: false,
      fileList: [],
      resultInfo:'',
      filePath: '',
  ...
  // 点击上传：手动上传到服务器，此时会触发组件的http-request
    ...
    recognize(){
      ...
      getIdCardImg(this.filePath).then(res=>{
        if(res.msg==='error'){
          this.$message.error(res.data);
        }else {
          this.$message.success(res.msg);
          console.log(res)
          this.resultInfo = '证件号码为：'+res.data.idNum;
