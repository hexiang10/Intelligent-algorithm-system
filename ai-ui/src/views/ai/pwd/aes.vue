  ...
  <div>
    <div>
      ...
      <h2 class="alert alert-success" style="font-size: 20px">
        AES（AdvancedEncryption
        Standard，高级数据加密标准）算法支持128位、192位和256位的秘钥长度，加密速度比DES和DESede都快，至今还没有被破解的报道。经过验证，目前采用的AES算法能够有效抵御已知的针对DES算法的所有攻击方法，如部分差分攻击、相关秘钥攻击等。AES算法因秘钥建立时间短、灵敏性好、内存需求低等优点，在各个领域得到广泛的研究与应用。
      </h2>
    </div>
    <el-row :gutter="15">
      <el-form
...
export default {
  ...
  methods: {
    encode() {
      ...
      this.formData.ciphertext = CryptoJS.AES.encrypt(
        this.formData.text,
    ...
    decode() {
      ...
      this.formData.text='';
      try{
        this.formData.text = CryptoJS.AES.decrypt(
