...
public class YoloController {
    ...
    @Autowired
    public YoloService yoloService;
    ...
    public AjaxResult upload(@RequestParam("file") MultipartFile file, Map<String, Object> map) {
        ...
        // 得到上传文件的路径
        String uploadPath = yoloService.upload(file, originalFileName);
        // 打印文件路径信息
        ...
        // 将每一个被识别的对象在图片框出来，并在框的左上角标注该对象的类别
         yoloService.markEveryDetectObject(src, results,map);
         log.info("map=>"+String.valueOf(map));
        // 将添加了标注的图片保持在磁盘上，并将图片信息写入map（给跳转页面使用）
        String outputName = yoloService.saveMarkedImage(src);
